...
function [V1,...
          ...
          exitflag] = lambert(r1vec,...
                              ...
                              tf,...
                              m,...
                              muC, unit_dist, unit_time) %#coder

    au = 149597870.7;

    % Convert au to km if inputs are in au units
    if strcmp(unit_dist, 'au')
        muC = muC * au^3;  % Convert mu to km^3/s^2 if input is in AU
        r1vec = r1vec * au;    % Convert position from AU to km
        r2vec = r2vec * au;
    elseif ~(strcmp(unit_dist, 'km') || strcmp(unit_dist, 'au'))
        error('Choose "au" or "km" as unit distance measurement');
    end

    if strcmp(unit_time, 'sec')
        tf = tf/60/60/24;
    elseif ~(strcmp(unit_time, 'sec') || strcmp(unit_time, 'days'))
        error('Choose "sec" or "days" as time measurement unit');
    end
