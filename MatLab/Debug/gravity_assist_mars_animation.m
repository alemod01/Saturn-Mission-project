%% ANIMAZIONE 2D/3D: Fly-by Marte 
try
    % --- SETUP PARAMETRI ---
    sat_pos_km = state_sat_mars_fb(:, 1:3); 
    sat_vel_km = state_sat_mars_fb(:, 4:6);
    ...
    max_safe_scale = (min_dist * 0.90) / R_mars;
    ...
    R_scaled = R_mars * planet_visual_scale;
    ...
    % --- SETUP GRAFICO ---
    figure('Name', 'Mars Fly-by', 'Color', 'k', 'Position', [100, 100, 900, 900]); 
    ...
    title({'KRONOS: Mars Gravity Assist'},'Color', 'w', 'FontSize', 14, 'FontWeight', 'bold', 'Interpreter', 'tex');
    ...
    Ym = Ym * R_scaled; 
    Zm = Zm * R_scaled;
    surf(Xm, Ym, Zm, 'EdgeColor', 'none', 'FaceColor', 'interp', 'DisplayName', 'Marte');

    colormap(gca, hot(100)); 
    caxis([-R_scaled, R_scaled * 3]); 
    
    % Illuminazione per dare volume
    ...
    plot3(visual_radius * cos(theta), visual_radius * sin(theta), zeros(size(theta)),  '--', 'Color', [1 0 1 0.4], 'LineWidth', 1, 'HandleVisibility', 'off');
    ...
    h_text_vel = text(0.5, 0.1, '', 'Units', 'normalized', 'Color', 'w', 'FontSize', 12, 'FontName', 'Courier', 'FontWeight', 'bold', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom'); 
